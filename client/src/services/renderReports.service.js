...
export function renderReports(reports) {
    ...
    const reportsContainer = document.getElementById(getSession().role == "alcaldia" && window.location.pathname == "/panel" ? "table-container" : "reports-container");

    let html = "";
    
    for (const report of reports) {
        html += `${getSession().role == "alcaldia" && window.location.pathname == "/panel" ? `
            <tr>
                ...
                <td class="py-3 pr-4 text-slate-500">${report.category}</td>
                <td class="py-3 pr-4"><span class="${getStatusColor(report.status)} text-white text-xs font-semibold px-2.5 py-1 rounded-full">${report.status}</span></td>
                <td class="py-3 pr-4 text-slate-500">${report.creationDate}</td>
            </tr>` : 
            
            `<a href="/report-detail" class="navigation bg-white rounded-xl shadow-card border border-slate-100 overflow-hidden hover:shadow-md transition group cursor-pointer">
                    <div class="relative h-32 bg-slate-200">
                        ...
                        <span class="absolute top-2 right-2 ${getStatusColor(report.status)} text-white text-[11px] font-semibold px-2 py-0.5 rounded-full">${report.status}</span>
                    </div>
                    <div class="p-3.5">
