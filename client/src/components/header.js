...
export function renderHeader() {
  ...
  const fullName = `${session.name[0].toUpperCase()}${session.name.slice(1)} ${session.lastName[0].toUpperCase()}${session.lastName.slice(1)}`;
    return `
        ...
        ${session.role == 'alcaldia' ? `<header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 gap-3 shrink-0 sticky inset-0 lg:left-64 z-1">
        ...
        </header>` : `<header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 gap-3 shrink-0 sticky inset-0 lg:left-64 z-1">
        <button class="menu-btn lg:hidden shrink-0 text-slate-500 hover:text-slate-700 cursor-pointer">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <line x1="4" x2="20" y1="12" y2="12" />
              <line x1="4" x2="20" y1="6" y2="6" />
